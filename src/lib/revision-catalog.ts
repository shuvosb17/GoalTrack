...
export type ReviewItemKind = "topic" | "module";
...
export function buildRevisionCatalog(
  ...
  for (const topic of topics) {
    if (topic.archived || !isTopicComplete(topic, subtopics)) continue;
  ...
  for (const mod of modules) {
    ...
    const modTopics = topics.filter(
      (t) => t.moduleId === mod.id && !t.archived && isTopicComplete(t, subtopics)
    );
    const confidences = modTopics.map(getTopicConfidence);
